...
<body>
			...
			<c:forEach var="book" items="${bookList}">
				<div class="box">
					<div class="pic" style="text-align: center">
						<a
							href="${pageContext.request.contextPath}/main/showBookDetail?id=${book.bookid}">
							<img src="${pageContext.request.contextPath}/${book.bookimage}">
						</a>
						<p class="name" ddt-src="${book.bookid}">
							<a title="${book.bookname}"
								href="http://product.dangdang.com/24161242.html" target="_blank"
								style=" ">${book.bookname}</a>
						</p>
						<p class="author">
							<span class="author_t"></span>${book.bookcreater}
						</p>
		...
		<div id="page-nav"
			...
			<ul class="pages" id="pageNav" tag-current="${currentPage}"
				...
				<c:if test="${currentPage > 1}">
					<li class="pgNext"><a
						href="${pageContext.request.contextPath}/main/listBookShowV2?keywords=${keywords}&currentPage=1"
						id="pageHome" name="pageHome">首页</a></li>
					<li class="pgNext"><a
						href="${pageContext.request.contextPath}/main/listBookShowV2?keywords=${keywords}&currentPage=${currentPage-1}"
						id="upPage" name="upPage">上一页</a></li>
				</c:if>

				<c:if test="${totalCount-currentPage >= 0}">
					<li class="page-number pgCurrent"><a
						href="main/listBookShowV2?keywords=${keywords}&currentPage=${currentPage}"
				...
				<c:if test="${totalCount-currentPage >= 2}">
					<li class="page-number"><a
						href="${pageContext.request.contextPath}/main/listBookShowV2?keywords=${keywords}&currentPage=${currentPage+2}"
						id="pageThree" name="pageThree">${currentPage+2}</a></li>
				</c:if>
				<c:if test="${totalCount-currentPage >= 3}">
					<li class="page-number"><a
						href="${pageContext.request.contextPath}/main/listBookShowV2?keywords=${keywords}&currentPage=${currentPage+3}"
						id="pageFour" name="pageFour">${currentPage+3}</a></li>
				</c:if>
				<c:if test="${totalCount-currentPage >= 4}">
					<li class="page-number"><a
						href="${pageContext.request.contextPath}/main/listBookShowV2?keywords=${keywords}&currentPage=${currentPage+4}"
						id="pageFive" name="pageFive">${currentPage+4}</a></li>
				</c:if>
				<c:if test="${totalCount-currentPage >= 5}">
					<li class="page-number"><a
						href="${pageContext.request.contextPath}/main/listBookShowV2?keywords=${keywords}&currentPage=${currentPage+5}"
						id="pageSix" name="pageSix">${currentPage+5}</a></li>
				</c:if>
				<c:if test="${totalCount-currentPage >= 6}">
					<li class="page-number"><a
						href="${pageContext.request.contextPath}/main/listBookShowV2?keywords=${keywords}&currentPage=${currentPage+6}"
				...
				<c:if test="${currentPage < totalCount}">
					...
					<li class="pgNext"><a
						href="${pageContext.request.contextPath}/main/listBookShowV2?keywords=${keywords}&currentPage=${totalCount}"
	...
	<script
		src="${pageContext.request.contextPath}/static/js/main/index.js"></script>
